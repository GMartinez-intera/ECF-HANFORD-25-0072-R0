...
swfl_path = os.path.join('..','stwd','sims','sc01','flmn')
hsfl_path = os.path.join('..','stwd','sims','sc01','flow')
eafl_path = os.path.join('.','sims','sc01','flow')
ebfl_path = os.path.join('.','comm','flow','base')
ncol = 188
nrow = 133
nlay = 8
nhsu = 7
...
'''
First = RectGrid()
First.readSPC(os.path.join(hsfl_path,'mod2tec','P2Rv9.1_SW.spc'))
Second = RectGrid()
Second.readSPC(os.path.join(ebfl_path,'P2Wv9.1.spc'))
...
for i in range(0, 101):
    Second.calcNewRealArray(First, os.path.join(swfl_path,'rch', 'rch_91_{0}.ref'.format(i)),
                               os.path.join(eafl_path,'rch','rch_91_{0}.ref'.format(i)), 1, "DOUBLE")
    yr+=1
...
#                               os.path.join(eafl_path,'rch','rech_{0}.ref'.format(i)), 1, "DOUBLE")
#    yr+=1
#
# Execute submodel
#print("Execute Submodels")
#command = 'mf2k-mst-cpcc09dpv.exe P2R.nam'
#os.chdir(eafl_path)
#p = Popen(command)
#os.chdir('..')
#p.wait()
