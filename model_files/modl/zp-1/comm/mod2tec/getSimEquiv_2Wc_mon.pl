...
@tmpE = ReadDelimFile("../../flow/model_top.ref",'\s+');
for ($j=1;$j<=$nc;$j++) {
  for ($i=1;$i<=$nr;$i++) {
    $lnode = (((($i-1)*$nc)+$j)-1);
...
for ($k=1;$k<=$nl;$k++) {
  @tmpK = ReadDelimFile("../../flow/hk_Layer_${k}.ref",'\s+');
  @tmpI = ReadDelimFile("../../flow/ibound_layer_${k}.ref",'\s+');
  @tmpE = ReadDelimFile("../../flow/botm_layer_${k}.ref",'\s+');
  for ($j=1;$j<=$nc;$j++) {
    for ($i=1;$i<=$nr;$i++) {
      $lnode = (((($i-1)*$nc)+$j)-1);
...
@aRefs = ReadDelimFileAoA("Monitoring_WELL_Screen_Info.txt",'\s+');
...
#read model results
@aRefs = ReadDelimFileAoA("P2R_cnc.out",'\s+');
...
open(OUTFIL,">P2R_cnc.smp");
$aCount=0;
@aRefs = ReadDelimFileAoA("P2R_cnc.tpl",'\s+');
foreach $aRef (@aRefs[0..$#aRefs]) {
  $aCount++;
  if ($aCount == 1) {
    printf OUTFIL "@{$aRef} \n";
