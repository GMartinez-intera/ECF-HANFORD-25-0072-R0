...
cht_list<-read.table("chart_mass_list.txt",header=TRUE,sep=',')
...
for (ppp in 1:length(zon_col)) {
  dat[[ppp]]<-read.table(paste(sim_path[ppp],"/","zone_mass_",fil_path[ppp],".out",sep=""),header = TRUE, sep=",")
}
...
for (ccht in cht_num) {
  ...
  IA<-dat[[sim_indx[1]]]
  IA<-IA[order(IA$ZONE_ID,IA$TIME),]
  ...
  for (indx in sim_indx) {
    IA<-dat[[indx]]
    IA<-IA[order(IA$ZONE_ID,IA$TIME),]
    tmpIndex = as.numeric(zon_col[indx])
    tmpCol = ((as.numeric(dat_col[indx])))
    zone_indx<-which(IA$ZONE_ID == tmpIndex)
    IA[,tmpCol]<-IA[,tmpCol]/divisor[indx]
    tmpMax=ceiling(max(IA[zone_indx,tmpCol],na.rm=TRUE))
  ...
  png(paste(sim_path[sim_indx],"/","mass",dat_col[sim_indx[1]],"_",fil_path[sim_indx[1]],"_zon",zon_col[sim_indx[1]],"_axs",axs_len[sim_indx[1]],".png",sep=""),width=3294,height=2580,res=300)
  #Build plot
...
#  y_minor<-c(seq(0.1,1,0.1),seq(1,10,1),seq(10,100,10),seq(100,1000,100),seq(1000,10000,1000),seq(10000,100000,10000),seq(100000,1000000,100000),seq(1.0E+5,1.0E+6,1.0E+5),seq(1.0E+6,1.0E+7,1.0E+6))
  ...
  for (indx in sim_indx) {
    ...
    zone_indx<-which(IA$ZONE_ID == tmpIndex)
    IA[,tmpCol]<-IA[,tmpCol]/divisor[indx]
    lines(IA$TIME[zone_indx]/365.25,IA[zone_indx,tmpCol],col=lin_color[indx],lwd=lin_width[indx],lty=lin_type[indx])
  }
  ...
  if (dat_col[sim_indx[1]] == 6) {
    mtext(text=paste("Total Mass Comparison\n",sep=""),side=3,font=2,cex=1.5,line=1)
  }
  if (dat_col[sim_indx[1]] == 8) {
    mtext(text=paste("Dissolved Mass Comparison\n",sep=""),side=3,font=2,cex=1.5,line=1)
  }
  if (dat_col[sim_indx[1]] == 9) {
    mtext(text=paste("Sorbed Mass Comparison\n",sep=""),side=3,font=2,cex=1.5,line=1)
  }
  #
  if (unit_title[indx]=="kg") {
    mtext(text=bquote(paste(.(axis_title[indx]),", kg")),side=2,font=2,cex=1.4,line=3)
  } else {
    mtext(text=paste(axis_title[indx],", Ci"),side=2,font=2,cex=1.4,line=3)
  } 
  #
