...
workspace = os.path.join(optm_dir,"tran","Transport_U_BP5")
mt = flopy.mt3d.mt.Mt3dms.load(namefile, model_ws=workspace, verbose=False)

##  CREATES MONTH FILES FOR USE WITH PLOTTING IN R
import pandas as pd
mnw2 = pd.read_table(os.path.join(str(optm_dir),"tran","Transport_U_BP5","ftl2ftl",well_out_filename),sep='\\s+')
up1_list = ['299-E33-268','299-E33-360','299-E33-361']
mnw2 = mnw2[mnw2['WELLID'].isin(up1_list)]
print(mnw2['WELLID'])
mnw2 = mnw2.drop(columns=['Seepage','elev.'])
ucn_file = flopy.utils.binaryfile.UcnFile(os.path.join(str(optm_dir),"tran","Transport_U_BP5","P2Ev9.1_20m_5th-243_u.ucn"), precision='double')
...
import pandas as pd
mas_obs = pd.read_table(os.path.join(str(optm_dir),"tran","Transport_U_BP5","well_by_well_observed.csv"),sep=',')
