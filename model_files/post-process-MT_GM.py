...
scenario = sys.argv[1]
optm_dir = os.path.join('modl','zp-1','sims', scenario)
well_out_filename = 'P2Wv9.1.BYND_out'
plot_title = 'CTET' + scenario
...
gridangle = 0.0
modelepsg = 32149
namefile = 'P2Wv9.1.nam'
workspace = os.path.join(optm_dir,"flow")
model_ws = workspace
...
namefile = 'P2RGWM.nam'
workspace = os.path.join(optm_dir,"tran")
#mt = flopy.mt3d.mt.Mt3dms.load(namefile, model_ws=workspace, verbose=False) # debug

##  CREATES MONTH FILES FOR USE WITH PLOTTING IN R
import pandas as pd
mnw2 = pd.read_table(os.path.join(str(optm_dir),"flow",well_out_filename),sep='\\s+')
mnw2 = mnw2.drop(columns=['Seepage','elev.'])
ucn_file = flopy.utils.binaryfile.UcnFile(os.path.join(str(optm_dir),"tran","P2RGWM.ucn"), precision='double')
...
mnw2['year']=((mnw2['full_time']/365.25)-0.05)+2012.0
...
allmnw2['Mass_kg']=0.0
allmnw2['Mass_kg']= -1*allmnw2['Q-node']*allmnw2['Concentration']*allmnw2['delt']/1e9
...
import pandas as pd
mas_obs = pd.read_table(os.path.join(str(optm_dir),"figs","well_by_well_observed.csv"),sep=',')
...
mas_obs_fyr = mas_obs.groupby(['fy_year'])['MASS_KG'].sum()
...
ax3.set_ylabel('Total Mass Extracted, kg',fontdict={'fontsize':10})
ax2 = fig.add_subplot(spec[3])
ax4 = mass_fyr.plot(fontsize=12,grid=True,secondary_y=True,color='black')
ax2.set_xlabel('Year',fontdict={'fontsize':10})
current_values = ax2.get_xticks()
ax2.set_xticklabels(['{:2.0f}'.format(x)[2:] for x in current_values])
ax4.scatter(mas_obs_fyr['fy_year'], mas_obs_fyr['MASS_KG'],s=14,facecolor='none',edgecolor='black',lw=0.5,marker="o") 
ax2 = fig.add_subplot(spec[2,:])
plt.axis('off')
handles, labels = ax3.get_legend_handles_labels()
ax2.legend(handles,labels,fontsize=11,ncol=len(handles))
fig.suptitle(plot_title, fontsize = 16)
plt.savefig( os.path.join(optm_dir,"tmpMassFlow_" + scenario + ".png"),dpi=300)
