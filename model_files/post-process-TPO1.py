...
workspace = os.path.join(optm_dir,"tran","Transport_Tc99_PO1")
...
up1_list = ['299-E25-240','299-E27-157']
mnw2 = mnw2[mnw2['WELLID'].isin(up1_list)]
print(mnw2['WELLID'])
mnw2 = mnw2.drop(columns=['Seepage','elev.'])
ucn_file = flopy.utils.binaryfile.UcnFile(os.path.join(str(optm_dir),"tran","Transport_Tc99_PO1","P2Ev9.1_20m_5th-243_Tc99_PO-1.ucn"), precision='double')
...
import pandas as pd
mas_obs = pd.read_table(os.path.join(str(optm_dir),"tran","Transport_Tc99_PO1","well_by_well_observed.csv"),sep=',')
